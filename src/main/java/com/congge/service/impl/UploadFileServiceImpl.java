...
import com.congge.entity.file.Constant;
import com.congge.entity.file.FileForm;
import com.congge.entity.file.UploadFile;
import com.congge.mapper.UploadFileMapper;
import com.congge.service.UploadFileService;
import com.congge.utils.FileMd5Util;
import com.congge.utils.KeyUtil;
import com.congge.utils.NameUtil;
...
public class UploadFileServiceImpl implements UploadFileService {
    ...
    public Map<String, Object> realUpload(FileForm form, MultipartFile multipartFile) throws Exception {
        ...
        if (path.isDirectory()) {
            ...
            if (fileArray != null) {
                if ((fileArray.length-1) == total) {
                    System.out.println("最后一次文件大小:" + multipartFile.getSize());
                    ...
                    updateWrapper.eq("file_status",2);
                    updateWrapper.eq("file_name",fileName);
                    updateWrapper.eq("file_md5",md5);
                    updateWrapper.eq("file_path",filePath);
                    updateWrapper.eq("file_suffix",suffix);
                    updateWrapper.eq("file_size",size);
                    uploadFileMapper.update(uploadFile,updateWrapper);
                    map=new HashMap<>();
                    map.put("fileId", fileId);
                    map.put("flag", "2");
                    return map;
